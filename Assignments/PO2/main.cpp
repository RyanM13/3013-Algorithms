...
*  Course:           CMPS 3013
*  Semester:         Sprint=g 2024
* 
*  Description:
*        This program uses getch and json to read in from the dictionary file
*        when you find one word it will print the definition below the word. 
* 
*  Usage:
*        
* 
 ...
 */
vector<string> partialMatch(json myjson, string substring) {
...
int main() {
  ...
  vector<string> animals = loadAnimalsFast();
  json myjson = loadJsonFile(filepath);
  ...
  while ((k = getch()) != 'Z') {
    ...
    // Find any word in the json file that partially matches
    // our substr word
    ...
    if ((int)k != 32) { // if k is not a space print it
      ...
      cout << style::bold << style::underline << fg::black
           ...
           << fg::reset << endl
           << endl;
      // resizing the vector to only print 10 matches instead of all of them
      if (matches.size() > 10)
        matches.resize(10);
      for (auto &c : substr)
        c = tolower(c);
      ...
      //     string key = element.key();
      //     string val = element.value();
      //     cout << "key:: " << key << "Def:: " << val << endl;
      //   }
      //}
      if (matches.size() == 1) {

        // myjson is a list of key == value pairs
        // cout<<myjson["buffalo"] this will print the def of buffalo
        // matches[0] = buffalo
        // myjson[matches[0]] does the same...
        // f(g())
        
        cout << fg::green << style::bold << "Def: " << style::reset << fg::yellow << myjson[matches[0]] << endl;
